...
func ResolveCommit(barePath, ref string) (string, error) {
	cmd := exec.Command("git", "-C", barePath, "rev-parse", ref)
	out, err := cmd.Output()
	if err != nil {
...
func Diff(worktreePath, baseCommit string) ([]DiffFile, error) {
	cmd := exec.Command("git", "-C", worktreePath, "diff", "--no-color", "-M", baseCommit)
	out, err := cmd.Output()
	if err != nil {
		// diff returns exit code 1 when there are differences, which is fine
		if exitErr, ok := err.(*exec.ExitError); ok && exitErr.ExitCode() == 1 {
			// still use the output
		} else {
...
func parseDiff(raw string) []DiffFile {
	...
	for i := 0; i < len(lines); i++ {
		...
		if line == "new file mode 100644" || strings.HasPrefix(line, "new file mode ") {
			current.Status = "added"
			continue
		}
		if line == "deleted file mode 100644" || strings.HasPrefix(line, "deleted file mode ") {
		...
		if strings.HasPrefix(line, "--- ") {
			if strings.HasPrefix(line, "--- a/") {
				...
				if current.OldPath == "" && current.Status == "modified" {
		...
		if strings.HasPrefix(line, "@@") {
			...
			oldNo = currentHunk.OldStart
			newNo = currentHunk.NewStart
			continue
		}

		// Skip other diff metadata lines (index, similarity, etc.)
		if currentHunk == nil {
			continue
		}
